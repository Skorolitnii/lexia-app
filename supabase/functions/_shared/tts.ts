...
/** Голоса Azure под язык изучения. Те же, что знает клиент. */
const VOICES: Record<string, string> = {
  en: "en-US-AvaMultilingualNeural",
  de: "de-DE-SeraphinaMultilingualNeural",
  it: "it-IT-IsabellaMultilingualNeural",
  fr: "fr-FR-DeniseNeural",
  es: "es-ES-ElviraNeural",
};

export function normalizeStudyLanguage(value: unknown): StudyLanguage {
  return value === "de" || value === "it" || value === "fr" || value === "es"
    ? value
    : "en";
}

export function cloudVoice(language: string): string {
  return VOICES[normalizeStudyLanguage(language)] ?? VOICES.en!;
}
  ...
  switch (status) {
    case 400:
      return new AzureError(
        status,
        "bad request (SSML or voice name)",
        false,
        detail,
      );
    case 401:
    case 403:
      return new AzureError(
        status,
        "key rejected - check AZURE_SPEECH_KEY/REGION",
        false,
        detail,
      );
    case 429:
      ...
      return new AzureError(
        status,
        "rate limited or monthly quota exhausted",
        true,
        detail,
      );
    case 408:
