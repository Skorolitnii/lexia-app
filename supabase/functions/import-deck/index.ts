...
async function warmAudio(
  ...
  await pool(jobs, CONCURRENCY, async ({ voice, text }) => {
    try {
      ...
      if (failed <= 3) {
        console.error(
          JSON.stringify({ at: "warm", chars: text.length, detail: String(e) }),
        );
      }
    }
  });
...
Deno.serve(async (req) => {
  ...
  if (userError || !userData.user) {
    console.error(
      JSON.stringify({ at: "auth", detail: userError?.message ?? "no user" }),
    );
  ...
  if (existingError) {
    console.error(
      JSON.stringify({ at: "existing", detail: existingError.message }),
    );
    return json({ error: "lookup failed" }, 500);
  }

  const taken = new Set(
    (existing ?? []).map((n) => duplicateKey(n.front as string)),
  );
  ...
  if (notesError) {
    console.error(
      JSON.stringify({ at: "insert-notes", detail: notesError.message }),
    );
  ...
  if (cardsError) {
    ...
    console.error(
      JSON.stringify({ at: "insert-cards", detail: cardsError.message }),
    );
