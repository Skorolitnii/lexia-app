...
import { deviceVoices, englishVoices, pickVoice, type VoiceLike } from '@/speech/voices'

const v = (lang: string, name: string, voiceURI = name): VoiceLike => ({ lang, name, voiceURI })
...
describe('pickVoice', () => {
  it('без выбора берёт пригодный английский голос', () => {
    expect(pickVoice([RU, ZARVOX, GOOGLE_UK_MALE], 'uk')).toBe(GOOGLE_UK_MALE)
  })

  it('предпочитает голос запрошенного акцента', () => {
    expect(pickVoice([GOOGLE_UK_MALE, GOOGLE_US], 'us')).toBe(GOOGLE_US)
  })
  ...
  it('предпочитает качественный голос компактному', () => {
    ...
    expect(pickVoice([compact, premium], 'us')).toBe(premium)
  })

  it('деградирует к акценту, если пригодных нет (одни novelty)', () => {
    expect(pickVoice([RU, ZARVOX], 'us')).toBe(ZARVOX)
  })

  it('деградирует к любому голосу, если английского нет вовсе', () => {
    expect(pickVoice([RU], 'us')).toBe(RU)
  })
  ...
  it('закреплённый голос перекрывает дефолт', () => {
    expect(pickVoice([GOOGLE_UK_MALE, DANIEL], 'us', 'com.apple.voice.daniel')).toBe(DANIEL)
  })

  it('игнорирует закреплённый голос, если его больше нет на устройстве', () => {
    // Голос удалили из ОС - не падаем, берём дефолт.
    expect(pickVoice([GOOGLE_UK_MALE], 'uk', 'com.apple.voice.fred')).toBe(GOOGLE_UK_MALE)
  })

  it('ловит Apple-голос под англоязычной локалью системы', () => {
    const daniel = v('en-GB', 'Daniel', 'com.apple.voice.daniel')
    expect(pickVoice([RU, daniel], 'uk')).toBe(daniel)
  })
})

describe('deviceVoices', () => {
  it('показывает английские голоса всех локалей', () => {
    ...
    // два имени.
    const list = deviceVoices([DANIEL, KAREN, v('en-IE', 'Мойра'), v('en-ZA', 'Тесса')])
    expect(list).toHaveLength(4)
  })

  it('всё ещё прячет novelty', () => {
    // По «Зарвоксу» произношение не учат ни на какой локали.
    expect(deviceVoices([ZARVOX, v('en-US', 'Колокольчик')])).toEqual([])
  })

  it('не пускает неанглийские локали', () => {
    expect(deviceVoices([RU, v('ja-JP', 'Flo (японский (Япония))', 'flo.ja')])).toEqual([])
  })
})

describe('englishVoices', () => {
  it('оставляет голоса US/UK, кроме novelty и старых compact', () => {
    // KAREN - en-AU, FRED - старый синтетический голос: оба мимо.
    const list = englishVoices([RU, DANIEL, GOOGLE_US, FRED, ZARVOX, KAREN])
    expect(list.map((x) => x.name).sort()).toEqual(['Google US English', 'Дэниэл'])
  })

  it('отсекает английский за пределами US/UK', () => {
    // Переключатель произношения знает только US и UK - остальным акцентам
  ...
  it('пропускает голос, которого не было в старом белом списке', () => {
    ...
    const allison = v('en-US', 'Allison (Enhanced)', 'com.apple.voice.enhanced.allison')
  ...
  it('дедупит один голос, продублированный под разными языками', () => {
    ...
    const dupB = v('en-GB', 'Google US English', 'google-us')
  ...
  it('держит US и UK версии одного голоса раздельно - это разные акценты', () => {
    const uk = v('en-GB', 'Eddy (английский (Великобритания))', 'eddy.uk')
    const us = v('en-US', 'Eddy (английский (Соединенные Штаты Америки))', 'eddy.us')
    expect(englishVoices([uk, us])).toHaveLength(2)
  })
  ...
  it('сортирует качественные голоса выше компактных', () => {
    ...
    expect(englishVoices([compact, enhanced, premium]).map((x) => x.name)).toEqual([
      'Zoe (Premium)',
      'Tom (Enhanced)',
      'Aaron',
    ])
  ...
  it('держит голоса Google наверху списка', () => {
    ...
    const list = englishVoices([DANIEL, v('en-US', 'Саманта'), GOOGLE_US, GOOGLE_UK_MALE])
    expect(list.slice(0, 2).map((x) => x.name)).toEqual([
  ...
  it('отсеивает novelty вместе с суффиксом качества', () => {
    expect(englishVoices([v('en-US', 'Whisper (Premium)'), v('en-US', 'Bells')])).toEqual([])
  })
  ...
  it('отсеивает Grandma/Grandpa/Rocko с языком в скобках', () => {
    const junk = [
      ...
      v('en-US', 'Grandpa (английский (Соединенные Штаты Америки))', 'grandpa.us'),
...
describe('englishVoices', () => {
  it('автоподбор держится US/UK', () => {
    // `englishVoices` кормит `pickVoice`: расширять его до прочих локалей
    // нельзя, иначе дефолтным голосом станет австралиец или ирландка.
    expect(englishVoices([KAREN, v('en-IE', 'Мойра')])).toEqual([])
  })
  ...
  it('неанглийские локали не проходят', () => {
    expect(englishVoices([RU, v('ja-JP', 'Flo (японский (Япония))', 'flo.ja')])).toEqual([])
  })
})
