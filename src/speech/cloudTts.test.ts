...
describe('cacheKey', () => {
  ...
  it('голос и скорость входят в ключ - облако запекает их в файл', () => {
    ...
    expect(cacheKey('otter', 'en-US-AvaMultilingualNeural', 1.25)).not.toBe(base)
  })
  ...
  it('совпадает с эталоном (та же формула в Edge Function)', () => {
    expect(cacheKey('  the   otter swims ', 'en-US-AvaMultilingualNeural', 1)).toBe(
      '55c25b879e2112c9',
    )
  })
  ...
  it('дрожь плавающей точки не создаёт файл-двойник', () => {
    expect(cacheKey('otter', 'v', 0.9500000000000001)).toBe(cacheKey('otter', 'v', 0.95))
  })
})

describe('cloudVoice', () => {
  it('регион озвучки выбирает акцент синтеза', () => {
    expect(cloudVoice('us')).toContain('en-US')
    expect(cloudVoice('uk')).toContain('en-GB')
  })
})
...
describe('storageUrl', () => {
  ...
  it('хвостовой слэш в базовом URL не даёт двойного', () => {
    expect(storageUrl('https://proj.supabase.co/', 'abc')).not.toContain('co//storage')
  })
})
...
describe('cachedAudioUrl', () => {
  ...
  it('адрес совпадает с тем, что вернёт синтез', async () => {
    ...
    const guess = cachedAudioUrl('the otter swims', 'us', 1, CONFIG)
    ...
    await synthesizeAudioUrl('the otter swims', 'us', 1, CONFIG, fetchImpl)
...
describe('synthesizeAudioUrl', () => {
  it('зовёт функцию и возвращает её адрес', async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValue(res(true, { url: 'https://proj.supabase.co/made.mp3' }))

    const url = await synthesizeAudioUrl('the otter swims', 'us', 1, CONFIG, fetchImpl)
  ...
  it('облако выключено - молча null, озвучит локальный синтез', async () => {
    ...
    expect(await synthesizeAudioUrl('the otter swims', 'us', 1, null, fetchImpl)).toBeNull()
  ...
  it('без входа функцию не зовём - она проверяет JWT сессии', async () => {
    ...
    expect(await synthesizeAudioUrl('the otter swims', 'us', 1, anon, fetchImpl)).toBeNull()
    expect(fetchImpl).not.toHaveBeenCalled()
  })

  it('офлайн - null, а не исключение: это повод озвучить локально', async () => {
    const fetchImpl = vi.fn().mockRejectedValue(new TypeError('Failed to fetch'))
    expect(await synthesizeAudioUrl('the otter swims', 'us', 1, CONFIG, fetchImpl)).toBeNull()
  })
  ...
  it('после 429 облако не трогаем - квота кончилась надолго', async () => {
    ...
    expect(await synthesizeAudioUrl('first phrase', 'us', 1, CONFIG, limited)).toBeNull()
    expect(limited).toHaveBeenCalledTimes(1)

    // Следующая фраза функцию уже не беспокоит.
    const after = vi.fn()
    expect(await synthesizeAudioUrl('second phrase', 'us', 1, CONFIG, after)).toBeNull()
  ...
  it('обычный отказ паузу не включает - осечка могла быть разовой', async () => {
    ...
    expect(await synthesizeAudioUrl('first phrase', 'us', 1, CONFIG, failed)).toBeNull()

    const after = vi.fn().mockResolvedValue(res(true, { url: 'https://proj.supabase.co/ok.mp3' }))
    expect(await synthesizeAudioUrl('second phrase', 'us', 1, CONFIG, after)).not.toBeNull()
  })

  it('пустой текст не тратит запрос', async () => {
    const fetchImpl = vi.fn()
    expect(await synthesizeAudioUrl('   ', 'us', 1, CONFIG, fetchImpl)).toBeNull()
    expect(fetchImpl).not.toHaveBeenCalled()
  })
})
