...
export async function importDeck(
  ...
  const body = (await callFunction('import-deck', { notes, folderId, region, rate })) as {
    created?: unknown
    skipped?: unknown
  }
  return {
 ...
 */
export function warmAudio(texts: string[], region: string, rate: number): void {
  if (!client || texts.length === 0) return
  void callFunction('warm-audio', { texts, region, rate }).catch(() => {})
}
