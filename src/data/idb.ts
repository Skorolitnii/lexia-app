...
const DB_NAME = 'lexia'
const DB_VERSION = 1
const STORES = ['folders', 'notes', 'cards', 'review_logs', 'settings'] as const
...
function openDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    req.onblocked = () => reject(new Error('IndexedDB заблокирована другой вкладкой приложения'))
  })
}
...
async function get<T>(db: IDBDatabase, store: StoreName, key: string): Promise<T | undefined> {
  return tx<T | undefined>(db, store, 'readonly', (os) => os.get(key))
}

async function put<T>(db: IDBDatabase, store: StoreName, value: T): Promise<void> {
 ...
 * без строки журнала.
 */
function putAll(db: IDBDatabase, writes: { store: StoreName; value: unknown }[]): Promise<void> {
  return new Promise((resolve, reject) => {
...
export class IdbRepository implements Repository {
  ...
  async listFolders(): Promise<FolderRow[]> {
    ...
    return rows.filter((r) => !r.deleted).sort((a, b) => a.position - b.position)
  }
  ...
  async folderNoteCounts(): Promise<{ folderId: string | null; count: number }[]> {
    const notes = (await getAll<NoteRow>(this.db, 'notes')).filter((r) => !r.deleted)
    const byFolder = new Map<string | null, number>()
    for (const n of notes) byFolder.set(n.folder_id, (byFolder.get(n.folder_id) ?? 0) + 1)
  ...
  async listCardsForNotes(noteIds: string[]): Promise<CardRow[]> {
    ...
    return (await getAll<CardRow>(this.db, 'cards')).filter((c) => !c.deleted && ids.has(c.note_id))
  }
  ...
  private async syncCards(note: NoteRow): Promise<void> {
    ...
    const cards = (await getAll<CardRow>(this.db, 'cards')).filter((c) => c.note_id === note.id)
  ...
  async countNewCardsIntroduced(now = new Date()): Promise<number> {
    ...
    const introduced = new Set(
      logs
        .filter((l) => l.state === State.New && new Date(l.review).getTime() >= start.getTime())
        .map((l) => l.card_id),
    )
  ...
  async getSettings(): Promise<SettingsRow> {
    ...
    if (existing) return { ...existing, tts_cloud: existing.tts_cloud ?? true }
  ...
  async replaceAll(data: {
    ...
    return new Promise((resolve, reject) => {
      ...
      const own = <T extends { user_id?: string }>(row: T) => ({ ...row, user_id: LOCAL_USER_ID })
      ...
      for (const row of data.review_logs) t.objectStore('review_logs').put(own(row))
