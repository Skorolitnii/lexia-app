...
import { buildQueue, queueCounts, queueOutlook, NO_FOLDER, type Scope } from '@/data/queue'
...
function card(id: string, note_id: string, over: Partial<CardRow> = {}): CardRow {
  return {
    id,
    user_id: 'u',
    note_id,
...
describe('buildQueue', () => {
  ...
  it('ограничивает новые остатком дневного лимита', () => {
    ...
    const cards = Array.from({ length: 10 }, (_, i) => card(`new${i}`, 'n1', { state: State.New }))
    expect(buildQueue(cards, notes, { ...opts, newCardsLeft: 3 })).toHaveLength(3)
  })
  ...
  it('исчерпанный лимит не пускает новые, но повторы остаются', () => {
    ...
    expect(buildQueue(cards, notes, { ...opts, newCardsLeft: 0 })).toHaveLength(1)
  })

  it('отрицательный остаток (лимит уменьшили после изучения) не ломает срез', () => {
    const cards = [card('new', 'n1', { state: State.New })]
    expect(buildQueue(cards, [note('n1', null)], { ...opts, newCardsLeft: -5 })).toEqual([])
  })
  ...
  it('cram берёт всё вне расписания, включая будущее и новые', () => {
    const notes = [note('n1', null)]
    const cards = [
      card('future', 'n1', { due: new Date(NOW.getTime() + 100 * DAY).toISOString() }),
      card('new', 'n1', { state: State.New }),
    ]
    const queue = buildQueue(cards, notes, { ...opts, cram: true, newCardsLeft: 0 })
...
describe('queueCounts', () => {
  it('делит очередь на новые и повторы', () => {
    const queue = [card('a', 'n1', { state: State.New }), card('b', 'n1'), card('c', 'n1')]
...
describe('queueOutlook', () => {
  ...
  it('считает новые за лимитом', () => {
    ...
    expect(queueOutlook(cards, notes, { ...opts, newCardsLeft: 1 }).newBeyondLimit).toBe(2)
  })

  it('резерв пуст, когда лимит покрывает все новые', () => {
    const notes = [note('n1', null)]
    const cards = [card('c1', 'n1', { state: State.New })]
    expect(queueOutlook(cards, notes, { ...opts, newCardsLeft: 20 }).newBeyondLimit).toBe(0)
  })
  ...
  it('перебор нормы добором не уводит резерв в минус', () => {
    ...
    expect(queueOutlook(cards, notes, { ...opts, newCardsLeft: -5 }).newBeyondLimit).toBe(1)
  })
  ...
  it('находит ближайший будущий срок', () => {
    ...
    const cards = [
      ...
      card('c2', 'n2', { state: State.Review, due: new Date(NOW.getTime() + DAY).toISOString() }),
    ]
    expect(queueOutlook(cards, notes, { ...opts, newCardsLeft: 0 }).nextDueAt).toEqual(
  ...
  it('игнорирует просроченные - они уже в очереди', () => {
    const notes = [note('n1', null)]
    const cards = [
      card('c1', 'n1', { state: State.Review, due: new Date(NOW.getTime() - DAY).toISOString() }),
    ]
    expect(queueOutlook(cards, notes, { ...opts, newCardsLeft: 0 }).nextDueAt).toBeNull()
  })

  it('не смотрит за пределы области', () => {
    const notes = [note('n1', 'f1'), note('n2', 'f2')]
    const cards = [card('c1', 'n1', { state: State.New }), card('c2', 'n2', { state: State.New })]
    const scope: Scope = { kind: 'folders', folderIds: ['f1'] }
    expect(queueOutlook(cards, notes, { scope, newCardsLeft: 0, now: NOW }).newBeyondLimit).toBe(1)
  })
})
