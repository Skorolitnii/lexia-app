 ...
 */
function stripServerTimes<T extends { created_at?: string; updated_at?: string }>(row: T) {
  const rest = { ...row }
...
export class SupabaseRepository implements Repository {
  ...
  async listFolders(): Promise<FolderRow[]> {
    return SupabaseRepository.unwrap(
      await this.db.from('folders').select('*').eq('deleted', false).order('position'),
    )
  }

  async listNotes(): Promise<NoteRow[]> {
    return selectAll<NoteRow>(() => this.db.from('notes').select('*').eq('deleted', false))
  }

  async listCards(): Promise<CardRow[]> {
    // Карточек вдвое больше заметок (обратные), лимит достигается раньше всего.
    return selectAll<CardRow>(() => this.db.from('cards').select('*').eq('deleted', false))
  }
  ...
  async listNotesPage(query: NotePageQuery): Promise<NoteRow[]> {
    ...
    return SupabaseRepository.unwrap(
      await q.order('front').range(query.offset, query.offset + query.limit - 1),
    ) as NoteRow[]
  }

  async folderNoteCounts(): Promise<{ folderId: string | null; count: number }[]> {
  ...
  async listCardsForNotes(noteIds: string[]): Promise<CardRow[]> {
    ...
    return selectAll<CardRow>(() =>
      this.db.from('cards').select('*').eq('deleted', false).in('note_id', noteIds),
    )
  }
  ...
  async listReviewLogs(): Promise<ReviewLogRow[]> {
    ...
    return selectAll<ReviewLogRow>(() => this.db.from('review_logs').select('*'))
  }
  ...
  async createNotes(
    inputs: Omit<NoteRow, 'user_id' | 'created_at' | 'updated_at' | 'deleted'>[],
  ): Promise<NoteRow[]> {
    ...
    const cards = notes.flatMap((note) => buildCardsForNote(note).map(stripServerTimes))
    if (cards.length) {
  ...
  async updateNote(id: string, patch: Partial<NoteRow>): Promise<NoteRow> {
    ...
    const next = SupabaseRepository.unwrap(
      await this.db.from('notes').update(patch).eq('id', id).select('*').single(),
    ) as NoteRow

    const typeChanged = patch.type !== undefined && patch.type !== prev.type
    const reverseChanged = patch.reverse !== undefined && patch.reverse !== prev.reverse
  ...
  private async syncCards(note: NoteRow): Promise<void> {
    ...
    for (const direction of wanted) {
      if (existing.some((c) => c.direction === direction && !c.deleted)) continue
      const buried = existing.find((c) => c.direction === direction)
      const [fresh] = buildCardsForNote({ ...note, type: 'basic', reverse: false })
    ...
    const extra = existing
      ...
      .map((c) => c.id)
    if (extra.length) {
      const { error } = await this.db.from('cards').update({ deleted: true }).in('id', extra)
      if (error) throw new Error(error.message)
    }
  }
  ...
  async deleteNote(id: string): Promise<void> {
    ...
    const cards = await this.db.from('cards').update({ deleted: true }).eq('note_id', id)
    if (cards.error) throw new Error(cards.error.message)
    const note = await this.db.from('notes').update({ deleted: true }).eq('id', id)
  ...
  async updateFolder(id: string, patch: Partial<FolderRow>): Promise<FolderRow> {
    return SupabaseRepository.unwrap(
      await this.db.from('folders').update(patch).eq('id', id).select('*').single(),
    )
  }
  ...
  async deleteFolder(id: string, withNotes = false): Promise<void> {
    if (withNotes) {
      ...
      const noteRows = await this.db.from('notes').select('id').eq('folder_id', id)
      if (noteRows.error) throw new Error(noteRows.error.message)
      const noteIds = (noteRows.data ?? []).map((n) => n.id)
      if (noteIds.length > 0) {
        const cards = await this.db.from('cards').update({ deleted: true }).in('note_id', noteIds)
        if (cards.error) throw new Error(cards.error.message)
      }
      const notes = await this.db.from('notes').update({ deleted: true }).eq('folder_id', id)
      if (notes.error) throw new Error(notes.error.message)
      const { error } = await this.db.from('folders').update({ deleted: true }).eq('id', id)
      if (error) throw new Error(error.message)
      return
    }
    ...
    const notes = await this.db.from('notes').update({ folder_id: null }).eq('folder_id', id)
    if (notes.error) throw new Error(notes.error.message)
    const { error } = await this.db.from('folders').update({ deleted: true }).eq('id', id)
    if (error) throw new Error(error.message)
  }
  ...
  async applyCardPatch(
    ...
    const next = SupabaseRepository.unwrap(
      await this.db.from('cards').update(patch).eq('id', cardId).select('*').single(),
    ) as CardRow
  ...
  async undoReview(card: CardRow, logId: string): Promise<void> {
    ...
    void logId
    const { error } = await this.db.from('cards').update(card).eq('id', card.id)
    if (error) throw new Error(error.message)
  }
