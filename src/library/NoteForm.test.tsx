...
const ENTRY = {
  hello: [{ word: 'hello', tags: ['ipa_pron:həlˈoʊ'], defs: ['n\tA greeting.'] }],
  bright: [{ word: 'bright', tags: ['ipa_pron:brˈaɪt'], defs: ['adj\tEmitting light.'] }],
...
function Harness({
  ...
  const applyToDraft = useCallback(
    (update: (current: NoteDraft) => NoteDraft) => setDraft((prev) => update(prev)),
    [],
  )
...
function renderForm(front: string) {
  ...
  const view = render(<Harness initial={latest} onDraft={(d) => (latest = d)} />)
...
describe('NoteForm - подстановка из словаря', () => {
  it('подставляет транскрипцию', async () => {
    ...
    await waitFor(() => expect(draft().transcription).toBe('/həlˈoʊ/'), { timeout: 3000 })
  ...
  it('слово без определений всё равно даёт транскрипцию (otter)', async () => {
    ...
    await waitFor(() => expect(draft().transcription).toBe('/ˈɑtɝ/'), { timeout: 3000 })
  })
  ...
  it('«не найдено» не роняет форму и оставляет поля пустыми', async () => {
    ...
    expect(await screen.findByText(/Нет в словаре/, {}, { timeout: 3000 })).toBeTruthy()
...
describe('NoteForm - ручные примеры', () => {
  it('свой пример переживает и лукап, и смену слова', async () => {
    ...
    let latest: NoteDraft = { ...emptyDraft(null), front: 'bright', examples: [mine] }
    render(<Harness initial={latest} onDraft={(d) => (latest = d)} />)

    await waitFor(() => expect(latest.transcription).toBe('/brˈaɪt/'), { timeout: 3000 })
    expect(latest.examples).toEqual([mine])

    // Меняем слово так же, как пользователь - печатая в поле, без перемонтажа.
    await userEvent.clear(screen.getByLabelText('Слово / фраза (EN)'))
    await userEvent.type(screen.getByLabelText('Слово / фраза (EN)'), 'hello')

    await waitFor(() => expect(latest.transcription).toBe('/həlˈoʊ/'), { timeout: 3000 })
    expect(latest.examples).toEqual([mine])
  })
})
...
describe('NoteForm - валидация обязательных полей', () => {
  ...
  function renderWith(draft: Partial<NoteDraft>, onSubmit = vi.fn()) {
    ...
    render(<Harness initial={initial} onDraft={() => {}} folders={[FOLDER]} onSubmit={onSubmit} />)
    return { onSubmit, save: () => screen.getByRole('button', { name: /Сохранить/ }) }
  }

  it('без перевода не даёт сохранить и говорит, чего не хватает', async () => {
    const { onSubmit, save } = renderWith({ front: 'otter', folder_id: 'f1', back: '' })
  ...
  it('перевод из пробелов не считается заполненным', () => {
    const { save } = renderWith({ front: 'otter', folder_id: 'f1', back: '   ' })
  ...
  it('заполненный перевод открывает сохранение и показывает итог', () => {
    const { save } = renderWith({ front: 'otter', folder_id: 'f1', back: 'выдра' })
...
describe('NoteForm - выбор значения у многозначного слова', () => {
  it('показывает список всех значений', async () => {
    ...
    expect(await screen.findByText('A container.', {}, { timeout: 3000 })).toBeTruthy()
  ...
  it('клик по значению переносит подсветку на него', async () => {
    ...
    const container = await screen.findByText('A container.', {}, { timeout: 3000 })
  ...
  it('длинный список сокращён до двух значений на часть речи', async () => {
    ...
    expect(await screen.findByText('To move swiftly.', {}, { timeout: 3000 })).toBeTruthy()
  ...
  it('«показать ещё» раскрывает полный список и сворачивает обратно', async () => {
    ...
    const more = await screen.findByText(/Показать ещё 2 значения/, {}, { timeout: 3000 })
