  ...
  if (state.notFound) {
    return (
      <p className="mt-1.5 text-[12.5px] text-faint">
        Нет в словаре - сохранится без транскрипции
      </p>
    );
  }
  if (state.failed) {
            ...
            <button
              ...
              className={`flex w-full flex-col gap-0.5 rounded-[10px] border px-2.5 py-2 text-left transition-colors ${
                active
                  ...
                  : "border-line bg-card hover:border-hint"
              }`}
            >
...
export function NoteForm({
  ...
  if (front.trim() === "")
    footerHint = isCloze ? "Введите предложение" : "Введите слово или фразу";
  else if (!clozeValid) footerHint = "Отметьте пропуск в предложении";
  else if (!backFilled)
    footerHint = isCloze ? "Введите перевод предложения" : "Введите перевод";
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 pb-6 lg:px-7">
        ...
        <div>
          <label htmlFor="note-front" className={labelCls}>
            {isCloze ? "Предложение с пропуском" : "Слово / фраза"}
          </label>
          <input
            id="note-front"
            value={draft.front}
            ...
            placeholder={
              isCloze ? "The fox is a {{cunning::хитрый}} animal." : "resilient"
            }
            className={field}
          />
          {isCloze && (
            <p
              className={`mt-1.5 text-[12.5px] ${clozeValid ? "text-faint" : "text-again"}`}
            >
              {clozeValid
          ...
          {!isCloze && draft.study_language === "en" && (
            <DictionaryHint state={dict} draft={draft} />
          )}
        ...
        <div>
          ...
          <div className="space-y-2">
            {draft.examples.map((ex, i) => (
              <div
                key={i}
                className="rounded-field border border-line bg-card p-2.5"
              >
      ...
      <div className="shrink-0 border-t border-line bg-surface px-5 pt-3 pb-5 lg:px-7 lg:pb-6">
        <p className="mb-2.5 text-[12.5px] font-semibold text-faint">
          {footerHint}
        </p>
        <button
          type="submit"
        ...
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-ink/25 px-6">
          <div className="w-full max-w-[340px] rounded-card bg-card p-5 shadow-phone">
            ...
            <p className="mt-1.5 text-[13px] leading-snug text-faint">
              Вместе с карточками и прогрессом повторений. Отменить будет
              нельзя.
            </p>
