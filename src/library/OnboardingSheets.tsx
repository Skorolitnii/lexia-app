...
export function OnboardingSheets({
  ...
  const [folderNew, setFolderName] = useState<{ name: string; pickFor: 'note' | 'import' } | null>(
    null,
  )
        ...
        {open === 'note' && (
          <NoteSheet onClose={close}>
            <NoteForm
              ...
              onSubmit={() => void save()}
              onCancel={close}
              onCreateFolder={(name) => setFolderName({ name, pickFor: 'note' })}
            />
          </NoteSheet>
        )}
      </AnimatePresence>
        ...
        {open === 'import' && (
          <NoteSheet wide onClose={close}>
            <ImportPanel
              ...
              onCreateFolder={(name) => setFolderName({ name, pickFor: 'import' })}
            />
          </NoteSheet>
        )}
      </AnimatePresence>
