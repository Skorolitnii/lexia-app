 ...
 * Итоговый экран должен знать, что очередь упёрлась в лимит, а не в отсутствие
 * карточек.
 */
...
function makeRepo(count: number, limit: number, introduced = 0): Repository {
  ...
  return {
    ...
    getSettings: vi
      .fn()
      .mockResolvedValue({ new_cards_per_day: limit } as SettingsRow),
...
describe("useStudySession - дневная норма", () => {
  ...
  it("рестарт не обходит дневную норму", async () => {
    const { result } = renderHook(() => useStudySession(), {
      wrapper: wrapper(makeRepo(50, 20)),
    });
