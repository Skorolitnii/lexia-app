...
import { buildQueue, queueCounts, queueOutlook, type QueueOutlook, type Scope } from '@/data/queue'
...
export interface StudySession {
  ...
  stats: { reviewed: number; correct: number; newLearned: number; elapsedMs: number }
  finished: boolean
  restart: () => void
  /** Что осталось за очередью: новые сверх лимита и ближайший срок. */
  outlook: QueueOutlook
  /**
   * Разово взять ещё `n` новых слов сверх дневной нормы. Осознанное действие
   * пользователя: норма защищает от вала повторений через 2-3 дня, поэтому
   * автоматически её не поднимаем.
   */
  studyMore: (n: number) => void
}

const GRADES: Grade[] = [Rating.Again, Rating.Hard, Rating.Good, Rating.Easy]

export function useStudySession(scope: Scope = { kind: 'all' }, cram = false): StudySession {
  const repo = useRepo()
  ...
  const [outlook, setOutlook] = useState<QueueOutlook>({ newBeyondLimit: 0, nextDueAt: null })
  /**
   * Сколько новых взято сверх дневной нормы по кнопке «Учить дальше». Живёт
   * до конца сессии: пересборка очереди должна помнить добор, иначе карточки
   * пропали бы обратно за лимит.
   */
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([cards, allNotes, allFolders, settings, introducedToday]) => {
        ...
        const newCardsLeft = settings.new_cards_per_day - introducedToday + extraNew
        ...
        setOutlook(queueOutlook(cards, allNotes, { scope: parsedScope, newCardsLeft }))
        ...
        setLoaded({
          notesById,
          folderName: allFolders.find((f) => f.id === firstFolderId)?.name ?? null,
  ...
  }, [repo, scopeKey, cram, reloadKey, extraNew])
  ...
  const rate = useCallback(
    (rating: Grade) => {
      ...
      const next = rating === Rating.Again ? [...rest, { ...card, ...cardPatch }] : rest
      setQueue(next)
  ...
  // Добор новых сверх нормы. Введённые карточки попадут в журнал и будут
  // учтены `countNewCardsIntroduced`, поэтому завтра норма их не «простит» -
  // тормоз нельзя обойти повторными нажатиями.
  const studyMore = useCallback((n: number) => setExtraNew((x) => x + n), [])
