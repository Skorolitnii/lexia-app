...
/**
 * Озвучка нужна `useImport` только ради региона и скорости - их он передаёт
...
describe('useImport - папка из колоды', () => {
  it('выбирает существующую папку, если имя совпало', async () => {
    ...
    const { result } = renderHook(() => useImport(), { wrapper: wrapper(repo) })
  ...
  it('предлагает имя из колоды, если такой папки ещё нет', async () => {
    ...
    await waitFor(() => expect(result.current.suggestedFolderName).toBe('Animals'))
