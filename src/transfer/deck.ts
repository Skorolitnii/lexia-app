...
function asExamples(v: unknown): Example[] {
  ...
  return v.flatMap((raw) => {
    ...
    if (!text) return [];
    const translation = asString(
      (raw as { translation?: unknown }).translation,
    );
...
function parseNote(
  ...
  if (rawType !== "basic" && rawType !== "cloze")
    return `неизвестный тип «${rawType}»`;
  ...
  const studyLanguage =
    asString(o.language) !== null
      ? normalizeStudyLanguage(asString(o.language))
      : deckLanguage !== "en"
        ? deckLanguage
        : detectStudyLanguage(front);
...
export function parseDeck(text: string): Deck {
  ...
  if (!Array.isArray(o.notes))
    throw new DeckParseError("В файле нет списка notes");
  const language = normalizeStudyLanguage(asString(o.language));

  const notes: DeckNote[] = [];
  const issues: DeckIssue[] = [];
  o.notes.forEach((raw, index) => {
    const parsed = parseNote(raw, language);
    if (typeof parsed === "string") issues.push({ index, reason: parsed });
    else notes.push(parsed);
  });

  return { folder: asString(o.folder), language, notes, issues };
}
...
export function needsLookup(note: DeckNote): boolean {
  return (
    note.study_language === "en" &&
    note.type === "basic" &&
    lookupTerm(note.front) !== null
  );
}
